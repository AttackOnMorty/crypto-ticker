...
final class MenuRowTextTests: XCTestCase {
    ...
    func testAssemblesExpectedText() {
        let row = MenuRowText.make(glyph: "●", icon: "₿", code: "BTC", name: "Bitcoin", price: "68,000", change: "+2.50%")
        XCTAssertEqual(row.text, "●\t₿ BTC\tBitcoin\t$68,000\t+2.50%")
    }

    func testStatusRangeIsLeadingGlyph() {
        let row = MenuRowText.make(glyph: "○", icon: "Ξ", code: "ETH", name: "Ethereum", price: "2,500", change: "-1.20%")
    ...
    func testChangeRangePointsAtChangeEvenWhenPriceIsAlsoPlaceholder() {
        ...
        let row = MenuRowText.make(glyph: "●", icon: "₿", code: "BTC", name: "Bitcoin", price: "—", change: "—")
